...
\definecolor{myblue}{RGB}{140, 200, 255}
\definecolor{mybeige}{RGB}{255, 184, 114}
...
\titleformat{\subsection}
  {\normalfont\fontsize{7pt}{8pt}\bfseries} % Change 14pt to your desired size
...
\begin{defn}[D 1.4 Linear Combinations]
    $\sum_{i=1}^n\lambda_i\bv{v}_i$ scaled combinations of $n$ vectors $\bv{v}_i$.
\end{defn}
...
\begin{defn}[D 1.7 Combination Types]
    \begin{props}
        ...
        \item \textbf{Conic:} $\lambda_j\geq 0$ for $j=1,2,\dots, n$
        \item \textbf{Convex:} \textit{Both} Affine and Conic.
    \end{props}
\end{defn}
}{
\begin{defn}[D 1.9 Scalar/Dot Product] \\
    $\bv{v}\cdot\bv{w}:=\sum_{i=1}^m\bv{v}_i\bv{w}_i$ \\
...
\begin{defn}[D 1.11 Euclidean norm, squared norm, unit vector]
    $\|\bv{v}\|:=\sqrt{\bv{v}\cdot\bv{v}}=\sqrt{\bv{v}^\top\bv{v}}$, \textbf{Squared Norm:} $\|\bv{v}\|^2:=\bv{v}^\top\bv{v}$, \textbf{Unit Vector:} $\|\bv{u}\|=1=\frac{\bv{v}}{\|\bv{v}\|}=\frac{1}{\|\bv{v}\|}\bv{v}$ (for any vector $\bv{v}\neq 0 )$
\end{defn}

\begin{lemma}[L 1.12 Cauchy-Schwarz inequality]
    $|\bv{v}\cdot\bv{w}|\leq\|\bv{v}\|\|\bv{w}\|$ for any two vectors  $\bv{v},\bv{w}\in\R^m$. \\
    Equality if $\bv{w}=\lambda\bv{v}$ or $\bv{v}=\lambda\bv{w}$.
\end{lemma}
...
\begin{defn}[D 1.21 Linear (in)dependence]
    ...
    \begin{compactoptions}
        ...
        \item There are no $\lambda_i$, such that $\sum_{i=1}^n\lambda_i\bv{v}_i=\bv{0}$.
    \end{compactoptions}

    For matrices: $\neg\exists\bv{x}\neq0:A\bv{x}=0$ (no column vector is a linear combination of the other)
\end{defn}
...
\begin{defn}[D 1.25 Span]
    Set of all linear combinations of a vector: \\
...
\begin{note}[Construction of vectors with standard unit vectors]
    Every vector in a vector space (D 4.1) can be written as: $\bv{u}=\sum_{i=1}^m\bv{u}_i\bv{e}_i$, where $\bv{e}$ is a standard unit vector (just one component being $1$, all others $0$).
\end{note}
...
\begin{defn}[D 2.1 Matrix]
    $A=[a_{ij}]_{i=1}^m,_{j=1}^n$ - $m$ rows, $n$ columns (first rows, then columns)
\end{defn}

\begin{defn}[D 2.2 Matrix addition, scalar multiplication]
    \textit{Addition:} $A+B=[a_{ij}+b_{ij}]_{i=1}^m,_{j=1}^n$, \\
    \textit{Scalar multiplication:} $\lambda A=[\lambda a_{ij}]_{i=1}^m,_{j=1}^n$
\end{defn}

\begin{defn}[D 2.3 Square matrices]
    \textit{Identity matrix}: quadratic matrix, diagonals $1$, $A=AI=IA$; \\
    \textit{Diagonal matrix:} $a_{ij}=0$ if $i\neq j$; \textit{Triangle matrix}: \textbf{lower} if $a_{ij}$, \textbf{upper} else; \\
...
\begin{defn}[D 2.4 Matrix-Vector Product]
    Rows of matrix $(m\times n)$ with vector ($n$ elements), i.e. $\bv{u}_1=\sum_{i=1}^m a_{1,i}\bv{v}_i, I\bv{x}=\bv{x}$; \textbf{Trace:} Sum of the diagonal entries of a matrix.
\end{defn}
...
\begin{defn}[D 2.9 Column space]
    $\textbf{C}(A):=\{A\bv{x}:\bv{x}\in\R^n\}$, i.e.: the span (set of all linear combinations) of the column vectors. 
\end{defn}
...
\begin{lemma}[L 2.11 Independent columns span the column space]
    Let $A$ be an $m\times n$ matrix with $r$ independent columns, and let $C$ be the $m\times r$ submatrix containing the independent columns. Then $\textbf{C}(A)=\textbf{C}(C)$.
\end{lemma}
...
\begin{defn}[D 2.14 Row Space]
    $\textbf{R}(A):=\textbf{C}(A^\top)$. With the \textit{row} rank of $A$, being the \textit{column} rank of $A^\top$.
\end{defn}

\begin{defn}[D 2.17 Nullspace]
    Nullspace contains all input vectors that lead to output vector $\bv{0}$. $\textbf{N}(A)=\{\bv{x}\in\R^ n:A\bv{x}=\bv{0}\}$.
    It can easily be obtained from RREF by setting $\bv{b}=0$ in $R\bv{x}=\bv{b}$. If there are any free variables, choose any real (or complex|) number satisfying the condition. \\
    ...
    We may then freely choose to the \textit{free variables} $x(Q)$. then find \textit{basis variables} $x(I)$ using the above, typically choose $e_1, \dots, e_k$ for $x(Q)$ to get the $k$th vector of basis ($k=$ number of columns of $Q$). Finally, combine the vectors into one.
\end{defn}

\begin{defn}[D 2.27 Kernel and Image]
    \begin{compactitem}
        \item \textbf{Kernel:} $\textbf{N}(A)=\textbf{Ker}(T):=\{\bv{x}\in\R^n:T(\bv{x})=\bv{0}\}\subseteq\R^n$ (If $A$ is the unique $m\times n$ matrix, such that $T=T_A$)
        \item \textbf{Image:} $\textbf{C}(A)=\textbf{Im}(T):=\{T(\bv{x}):\bv{x}\in\R^m\}\subseteq\R^m$ (If $A$ is the unique $m\times n$ matrix, such that $T=T_A$), the set of all outputs that $T$ can produce.
    \end{compactitem}
\end{defn}
...
\begin{theorem}[T 2.46 CR Decomposition]
    ...
    \textbf{RREF} is simply REF without any zero rows. (i.e. in $R_0, R$ (in RREF would be $R_0$  without the last row), the leading entries in each non-zero row is $1$, and each leading $1$ is the \textit{only} non-zero entry in its column.
\end{theorem}

\section{Linear Equations}

\begin{note}
    Get the system of linear equations into a $A\bv{x}=\bv{b}$ form, which can then by solved by Gaussian elimination.
    \textbf{Gaussian elimination:} Transform $A$ into an \textit{upper}-triangle matrix by performing row operations on it (into RREF). \\
    First, augment $A$ by $b$, then perform Gaussian elimination, and finally solve by back-substitution. Gaussian elimination succeeds iff. $A\in\R^{m\times m}$ and the columns of $A$ are linearly independent (Runtime $\mathcal{O}(m^3)$).
\end{note}
...
\begin{defn}[D 3.13 Reduced Row Echelon Form]
    Let $R=[r_{ij}]_{i=1}^m,_{j=1}^n$ be an $m\times n$ matrix. $R$ is in RREF, if there is some natural number $r\leq m$ and column indices $1\leq j_1\leq j_2\leq\dots\leq n$ (*the indices of the “downward step”) such that the following two conditions hold:
    \begin{props}
...
\begin{note}[Gauss-Jordan elimination]
    ...
    \begin{compactitem}
        \item \textbf{No Solution:} $0=c$ (where $c\neq0$)
...
\begin{defn}[D 4.8 Subspace]
    Let $V$ be a vector space. A nonempty subset $U\subseteq V$ is a subspace of $V$ is these two axioms are true $\forall\bv{v},\bv{w}\in U$ and $\forall\lambda\bv{v}\in U$:
    $$\bv{v}+\bv{w}\in U\quad\lambda\bv{v}\in U$$
    They guarantee that vector addition and scalar multiplication doesn't take us outside the subspace.
\end{defn}
...
\begin{lemma}[L 4.11 Column space is a subspace]
    Let $A\in\R^{m\times n}$, then $\textbf{C}(A)=\{A\bv{x}:\bv{x}\in\R^n\}$ is a subspace of $\R^m$. Moreover, $\textbf{R}(A)=\textbf{C}(A^\top)$ is a subspace of $\R^n$.
\end{lemma}

\begin{lemma}[E 4.13 Nullspace is a subspace]
    Let $A\in\R^{m\times n}$. Then the nullspace $\textbf{N}(A)=\{\bv{x}\in\R^n:A\bv{v}=0\}$ is a subspace of $\R^n$.
\end{lemma}
...
\begin{lemma}[L 4.19 Independent columns is a basis]
    Set of vectors that are linearly independent and span $B$, the subspace of $V$. For $\R^m$, the set of unit vectors is a basis. For a matrix, all linearly independent columns form a basis of the column space $\textbf{C}(A)$.
    \textbf{Calculating:} If we have a matrix with full column / row rank, then the basis are all column / row vectors.
\end{lemma}
...
\begin{lemma}[L 4.23 Steinitz exchange lemma]
    Let $F\subseteq V$ be a finite set of linearly independent vectors, $G\subseteq V$ a finite set of vectors with $\text{Span}(G)=V$. Then: $|F|\leq|G|,\exists$ subset $E\subseteq G$ of size $|G|-|F|$ such that $\text{Span}(F\cup E)=V$.
\end{lemma}

\begin{theorem}[T 4.24 All bases have the same size]
    All bases have the same size: $B,B'\in V\Rightarrow|B|=|B'|$.
\end{theorem}
...
\begin{theorem}[T 4.31 Basis of $\textbf{C}(A)$: Pivots columns of RREF]
    If $R$ is RREF of $A$, then all columns at pivots of $R$ form a basis of $\textbf{C}(A):\text{dim}(\textbf{C}(A))=\text{rank}(A)=r$
\end{theorem}
...
\begin{corollary}[C 4.34 Rank is at most $\text{min}$ of the matrix dimensions]
    $A$ is a $m\times  n$ matrix with rank $r\Rightarrow r\leq \text{min}(n,m)$.
\end{corollary}

\begin{lemma}[L 4.35 Nullspace isomorphism]
    $R = \text{RREF}(A)$, then $T:N(R)\rightarrow\R^{n-r}$ is an isomorphism between $N(R)$ and $\R^{n-r}\rightarrow \text{dim}(N(R))=n-r$.
\end{lemma}
...
\begin{theorem}[T 4.39 Dimension of a solution space]
    Let $A\in\R^{m\times n}$ with rank $r$. If $A\bv{x}=\bv{b}$ is solvable, then: $\text{dim}(\textbf{Sol}(A,\bv{b}))=n-r$, and $\text{dim}(\textbf{Sol}(A,\bv{b})):=\text{dim}(N(A))$
\end{theorem}

\begin{theorem}[T 4.40 Systems of rank $m$ are solvable]
    Let $A\in\R^{m\times n}$ with $\text{rank}(A)=m,A\bv{x}=\bv{b}$ is solvable for all $\bv{b}\in\R^m$.
\end{theorem}
...
\begin{defn}[D 4.42 Types of systems]
    Let $A\in\R^{m\times n}$ and $\bv{b}\in\R^m$. The system $A\in\R^{m\times n}$ is called:
    \begin{compactitem}
        \item $m=n\Rightarrow$ square ($A$ is a square matrix) — \textbf{typically solvable}.
        \item $m<n\Rightarrow$ underdetermined ($A$ is a wide matrix) — \textbf{typically solvable}.
...
\begin{defn}[D 5.1.1 Orthogonal subspaces]
    Two vectors are orthogonal if their scalar product is $0:\bv{v}^\top\bv{w}=\sum_{i=1}^n \bv{v}_i\bv{w}_i=0$. Two subspaces are orthogonal if all $\bv{v}$ and $\bv{w}$ are orthogonal.
\end{defn}
...
\begin{corollary}[C 5.1.4 Combinations of subspaces]
    Let $V$ and $W$ be orthogonal subspaces. Then $V+W$ is a subspace of $\R^n$, and $V\cap W=\{\bv{0}\}$ and their union is $V\cup W=\{\lambda\bv{v}+\mu\bv{w}:\lambda,\mu\in\R,\bv{v}\in V,\bv{w}\in W\}$. If $\text{dim}(V)=k$ and $\text{dim}(W)=l$, then $\text{dim}(V\cup W)=k+l\leq n$, for $V,W\subseteq \R^n$.
\end{corollary}
...
\begin{lemma}[L 5.2.2 One-dimensional projection formula]
    Projection of $\bv{b}$ on $S=\{\lambda\bv{a}|\lambda\in\R\}=C(\bv{a}):\text{proj}_S(\bv{b})=\frac{\bv{a}\bv{a}^\top}{\bv{a}^\top\bv{a}}\bv{b}$, where $\bv{a}\in\R^m\backslash\{\bv{0}\}$. We note, that $(\bv{b}-\text{proj}_S(\bv{b}))\perp\text{proj}_S(\bv{b})$, i.e. the “error-vector” is perpendicular.
\end{lemma}

\begin{lemma}[L 5.2.3 General Projection Formula]
    Let $S$ be a subspace in $\R^m$ with basis $\bv{a}_1,\dots,\bv{a}_n$ that span $S$. Let $A$ be the matrix with column vectors $\bv{a}_1,\dots,\bv{a}_n$. The general formula: $\text{proj}_S(\bv{b})=A\hat{\bv{x}}$, where $\hat{\bv{x}}$ is $A^\top A\hat{\bv{x}}=A^\top\bv{b}$.
\end{lemma}
...
\begin{theorem}[T 5.2.5 Projection in terms of projection matrix]
    $\text{proj}_S(\bv{b})=P\bv{b}$ with projection matrix $P=A(A^\top A)^{-1}A^\top$. $A$ is a matrix given in a task, but has to have linearly independent columns.
\end{theorem}
...
\begin{defn}[D 6.3.8 Gram-Schmidt algorithm]
    This algorithm is used to construct orthonormal bases. We have linearly independent vectors $\bv{a}_1,\dots,\bv{a}_n$, that span a subspace $S$, then Gram-Schmidt constructs $\bv{q}_1,\dots,\bv{q}_n$ by:
    \begin{compactlist}
...
\begin{defn}[D 6.4.7 CR Decomposition with pseudoinverse]
    For $A\in\R^{m\times n}$ with $\text{rank}(A)=r$ and a \textit{CR}-decomposition $A=CR$, we define $A^\dagger=R^\dagger C^\dagger$. \\
    Which can be written as, $A^\dagger=R^\top(RR^\top)^{-1}(C^\top C)^{-1}C^\top=R^\top(C^\top CRR^\top)^{-1}C^\top=R^\top(C^\top AR^\top)^{-1}C^\top$.
\end{defn}

\begin{lemma}[L 6.4.8 Unique solution of least squares with pseudoinverse]
    For any matrix $A$ and vector $\bv{x}\in\mathcal{C}(A)$, then unique solution of the least squares problem is given by a vector $\hat{\bv{x}}\in\mathcal{C}(A)$ satisfying $A\hat{\bv{x}}=\bv{b}$. The solution is $\hat{\bv{x}}=A^\dagger\bv{b}$, with $A\hat{\bv{x}}=\bv{b}$, and in the general case $A^\dagger=R^\top C^\top=R^\top(C^\top AR^\top)^{-1}C^\top$.
\end{lemma}
...
\begin{theorem}[T 6.4.10 Properties of Pseudoinverse]
    ...
    \begin{compactitem}
        ...
        \item $AA^\dagger$ is symmetric, and the projection matrix for the projection on $\mathcal{C}(A)$.
        \item $A^\dagger A$ is symmetric, and the projection matrix for the projection on $\mathcal{C}(A^\top)$.
    \end{compactitem}

    Moreover,
    $$AA^\dagger=CRR^\top(RR^\top)^{-1}(C^\top C)^{-1}C^\top = C(C^\top C)^{-1}C^\top=(C(C^\top C)^{-1}C^\top)^\top$$, which is the projection onto $\mathcal{C}(A)$, and $(AA^\dagger)^\top=AA^\dagger$.
\end{theorem}
...
\begin{defn}[D 7.2.1 Permutation sign]
    ...
    $$\text{sgn}(\sigma)=\begin{cases}
        1 & \textit{if } |(i,j)\in\{1,\dots,n\}\times\{1,\dots,n\} \textit{ such that } i<j \textit{ and } \bv{\sigma}(i)>\bv{\sigma}(j)| \textit{ even} \\
        -1 & \textit{if } |(i,j)\in\{1, \dots, n\}\times\{1, \dots, n\} \textit{ such that } i<j \textit{ and } \bv{\sigma}(i) > \bv{\sigma}(j)| \textit{odd}
    \end{cases}$$
\end{defn}
...
\begin{note}[Determinant Properties]
    \begin{compactlist}
        \item Matrix $T\in\R^{n\times n}$ is triangular, then $\det(T)=\prod_{k=1}^n T_{kk}$, in particular $\det(I)=1$.
        \item Matrix $A\in\R^{n\times n}, \det(A)=\det(A^\top)$
        \item Matrix $Q\in\R^{n\times n}$ is orthogonal $\implies\det(Q)=1$ or $\det(Q)=-1$.
...
\begin{obs}[P 7.2.4 Determinant of orthogonal matrices]
    \begin{compactoptions}
        \item Given a permutation matrix $P\in\R^{m\times n}$ corresponding to a permutation $\bv{\sigma}$, then $\det(P)=\text{sgn}(\sigma)$. We sometimes also write $\text{sgn}(P)$.
    ...
    $1=\det(I)=\det(Q^\top Q)=\det(Q^\top)\det(Q)=\det(Q)^2$, so $\det(Q)=\pmb{1}$. If the determinant is $1$, then $Q$ is a rotation matrix. If the determinant is $-1$ it's a reflection matrix.
\end{obs}

\begin{obs}[P 7.3.2 Cofactor determinant calculation]
    Let $A\in\R^{n\times n}$, for any $1\leq i\leq n$, $\det(A)=\sum\limits_{j=1}^nA_{ij}C_{ij}$, where the co-factors are $C_{ij}=(-1)^{i+j}\det(A_{ij})$
\end{obs}

\begin{obs}[P 7.3.5 Cramer's Rule]
    The idea here is that we solve a linear system of type $A\bv{x}=\bv{b}$, then, due to the determinant being multiplicative, we get $\det(A)\bv{x}_1=\det(\mathscr{B})$, where $\mathscr{B}$ is the matrix obtained from $A$ by replacing the first column with $\bv{b}$. So, the solution $\bv{x}\in\R^n$ for $A\bv{x}=\bv{b}$ is given by $\bv{x}_j=\frac{\det(\mathscr{B})}{\det(A)}$
\end{obs}
...
\begin{note}[Operations]
    $i^2=-1$ (\textbf{NOT} $i=\sqrt{-1}$). Let $z_1=a_1+b_1i$ and $z_2=a_2+b_2i$.
    \begin{compactlist}
        \item \textbf{Addition/Subtraction}: $(a_1\pm a_2)+(b_1\pm b_2)i$ \textit{Ex:} $(3+2i) + (1-4i) = (3+1) + (2-4)i = \mathbf{4-2i}$
        \item \textbf{Multiplication}: $(a_1a_2-b_1b_2)+(a_1b_2+a_2b_1)i$ \textit{Ex:} $(3+2i)(1-4i) = (3(1)-2(-4)) + (3(-4)+1(2))i = (3+8) + (-12+2)i = \mathbf{11-10i}$
        \item \textbf{Division}: $\frac{a_1a_2+b_1b_2}{a_2^2+b_2^2}+\frac{a_2b_1-a_1b_2}{a_2^2+b_2^2}i$ \textit{Ex:} $\frac{3+2i}{1-4i} = \frac{3(1)+2(-4)}{1^2+(-4)^2} + \frac{1(2)-3(-4)}{1^2+(-4)^2}i = \frac{3-8}{1+16} + \frac{2+12}{1+16}i = \mathbf{\frac{-5}{17} + \frac{14}{17}i}$
    \end{compactlist}
\end{note}
...
\begin{note}[R 8.1.1 Euler's formula]
    For $\theta\in\R,e^{i\theta}=\cos\theta+i \sin\theta\Rightarrow e^{i\pi}+1=0$
\end{note}
...
\begin{lemma}[L 8.2.3 Real Eigenvalues / Eigenvectors]
    Let $A\in\R^{n\times n}$. Then, $\lambda\in|R$ is a real eigenvalue of $A$ if and only if $\det(A-\lambda I)=0$. A vector $\bv{v}\in\R^n\backslash\{0\}$ is an eigenvector associated with $\lambda$ if and only if $\bv{v}\in\mathcal{N}(A-\lambda I)$.
\end{lemma}
...
\begin{obs}[P 8.2.7 Eigenvalue of orthogonal matrix]
    If $Q\in\R^{n\times n}$ be an orthogonal matrix. If $\lambda\in\C$ is an eigenvalue of $Q$, then $|\lambda|=1$.
\end{obs}
...
\begin{theorem}[T 8.3.3 Existence of basis from Eigenvalues]
    Let $A\in\R^{n\times n}$ with $n$ distinct real eigenvectors. Then there exists a basis of $\R^n,\bv{v}_1,\dots,\bv{v}_n$ made of eigenvectors of $A$.
\end{theorem}
...
\begin{defn}[D 9.1.10 Geometric multiplicity]
    Let $A\in\R^{n\times n}$ and let $\lambda$ be an eigenvalue of $A$. Then $\text{dim}(\mathcal{N}(A-\lambda I))$ is called the \textit{geometric multiplicity} of $\lambda$.
\end{defn}
...
\begin{corollary}[C 9.2.2 Eigendecomposition]
    For any symmetric matrix $A\in\R^{n \times n}$, there exists and orthogonal matrix $V\in\R^{n\times n}$ (whose columns are eigenvectors of $A$) such that $A=V\Lambda V^\top$, where $\Lambda\in\R^{n\times n}$ is diagonal with diagonal entries equal to the eigenvalues of $A$, and $V^\top V=I$. This decomposition is called the \textit{eigendecomposition}.
\end{corollary}
...
\begin{corollary}[C 9.2.4 Rank of real symmetric matrices]
    \begin{compactitem}
        ...
        \item For a general $n\times n$ matrix, $\text{rank}(A)=n-\text{dim}(\mathcal{N}(A))$, so the geometric multiplicity of the eigenvalue $\lambda=0$ equals $\text{dim}(\mathcal{N}(A))$.
    \end{compactitem}
\end{corollary}
...
\begin{obs}[P 9.2.12 Positivity of the quadratic form]
    A symmetric matrix $A\in\R^{n\times n}$ is PSD iff. $\bv{x}^\top A\bv{x}\geq 0$ for all $\bv{x}\in\R^n$. Analogously, its PD iff. $\bv{x}^\top A\bv{x}> 0$ for all $\bv{x}\in\R^n\backslash\{0\}$.
\end{obs}

\begin{defn}[D 9.2.13 Gram Matrix]
    Given vectors $\bv{x}_1,\dots,\bv{v}_n\in\R^n$, their \textit{Gram matrix}  is $G\in\R^{n\times n}$ defined by $G_{ij}=\bv{v}_i^\top\bv{v}_j$. We have $i.j\leq n$ because $G\in\R^{n\times n}$. If $V=[\bv{v}_1\dots\bv{v}_n]\in\R^{m\times n}$, then $G=V^\top V$. \\
...
\begin{note}[General Solution]
    \begin{compactlist}
        ...
        \item \textbf{Consistency Check:} If any row looks like $[0\dots 0|non-zero$, there is \textbf{no solution}.
...
\begin{note}[Quick Rank Reference]
    ...
    \begin{compactlist}
        ...
        \item $\text{rank}(A) = r \Rightarrow \dim(\textbf{C}(A)) = r, \dim(\textbf{R}(A)) = r$
...
\begin{note}[Least Squares Approximation]
    ...
    \begin{compactlist}
        ...
        \item Calculate vector $d=A^\top\bv{b}$
...
\begin{obs}[Symmetric Matrices ($A=A^\top$)]
    \begin{compactitem}
        ...
        \item Always orthogonally diagonalizable.
...
\begin{obs}[Orthogonal Matrices ($Q^\top Q = I$)]
    \begin{compactitem}
        \item Rows and columns form an orthonormal basis.
        \item $Q^{-1} = Q^\top$.
        \item $\det(Q) = \pm 1$.
        \item $|\lambda| = 1$ (Eigenvalues lie on the unit circle in $\C$).
        \item Preserves dot products: $(Q\bv{x}) \cdot (Q\bv{y}) = \bv{x} \cdot \bv{y}$.
        \item Preserves norms (lengths): $\|Q\bv{x}\| = \|\bv{x}\|$.
    \end{compactitem}
\end{obs}
...
\begin{obs}[Skew-Symmetric Matrices ($A^\top = -A$)]
    \begin{compactitem}
        ...
        \item $\det(A) \geq 0$ (if $n$ is even), $\det(A)=0$ (if $n$ is odd).
        \item Eigenvalues are purely imaginary or zero.
        \item $\bv{x}^\top A \bv{x} = 0$ for all real vectors $\bv{x}$.
    \end{compactitem}
\end{obs}

\begin{obs}[Projection Matrices ($P^2 = P = P^\top$)]
    \begin{compactitem}
        \item Eigenvalues are only $0$ or $1$.
        \item If $P$ projects onto $S$, then $I-P$ projects onto $S^\perp$.
        \item Rank(P) = Dimension of subspace projected onto.
    \end{compactitem}
\end{obs}

\begin{obs}[Positive Definite Matrices]
    \begin{compactitem}
        \item $\bv{x}^\top A \bv{x} > 0$ for all non-zero $\bv{x}$.
        \item All eigenvalues $> 0$.
        \item All pivots $> 0$.
        \item Determinant $> 0$.
        \item $A^{-1}$ is also Positive Definite.
    \end{compactitem}
\end{obs}
...
\begin{note}[Rank Properties]
    \begin{compactitem}
        \item $\text{rank}(A) = \text{rank}(A^\top) = \text{rank}(A^\top A) = \text{rank}(AA^\top)$.
        \item $A$ is invertible $\iff$ Full Rank.
        \item Full column rank $\implies$ injective (one-to-one).
        \item Full row rank $\implies$ surjective (onto).
    \end{compactitem}
\end{note}
...
\begin{note}[Trace Tricks]
    \begin{compactitem}
        ...
        \item $\text{Tr}(ABC) = \text{Tr}(BCA) = \text{Tr}(CAB)$ (Cyclic property).
        \item $\text{Tr}(A^\top A) = \sum_{i,j} a_{ij}^2$ (Sum of all squared entries).
    \end{compactitem}
\end{note}

\begin{note}[Block Matrices]
    For block matrix $M = \begin{bmatrix} A & B \\ 0 & D \end{bmatrix}$ (Block Triangular):
    \begin{compactitem}
        \item $\det(M) = \det(A) \cdot \det(D)$.
        \item Eigenvalues of $M$ = Eigenvalues of $A$ $\cup$ Eigenvalues of $D$.
        \item If $A, D$ invertible: $M^{-1} = \begin{bmatrix} A^{-1} & -A^{-1}BD^{-1} \\ 0 & D^{-1} \end{bmatrix}$.
    \end{compactitem}
    For Block Diagonal $M = \text{diag}(A, D)$: $M^k = \text{diag}(A^k, D^k)$.
\end{note}

\begin{note}[Cross Product (in $\R^3$)]
    $\bv{u} \times \bv{v} = \det \begin{bmatrix} \bv{i} & \bv{j} & \bv{k} \\ u_1 & u_2 & u_3 \\ v_1 & v_2 & v_3 \end{bmatrix}$.
    \begin{compactitem}
        \item Result is orthogonal to both $\bv{u}$ and $\bv{v}$.
        \item $\|\bv{u} \times \bv{v}\|$ = Area of parallelogram spanned by $\bv{u}, \bv{v}$.
        \item $\bv{u} \cdot (\bv{v} \times \bv{w}) = \det([\bv{u}, \bv{v}, \bv{w}])$ (Scalar Triple Product / Volume).
    \end{compactitem}
\end{note}
...
\begin{note}[Quick Eigenvalue Checks]
    ...
    \begin{compactitem}
        \item $\lambda_1 + \lambda_2 = \text{Tr}(A)$.
        \item $\lambda_1 \cdot \lambda_2 = \det(A)$.
    \end{compactitem}
    If row sums are constant $s$: $s$ is an eigenvalue with eigenvector $[1, 1, \dots, 1]^\top$.
    If column sums are constant $s$: $s$ is an eigenvalue (of $A^\top$ and thus $A$).
\end{note}

\begin{note}[Nilpotent \& Idempotent]
    \begin{compactitem}
        \item \textbf{Nilpotent:} $A^k = 0$ for some $k$. All eigenvalues are $0$. $\det(A)=0, \text{Tr}(A)=0$.
        \item \textbf{Idempotent:} $A^2 = A$ (like projections). Eigenvalues are only $0$ or $1$.
    \end{compactitem}
\end{note}
