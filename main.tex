...
\begin{lemma}[L 1.28 Span of $m$ linearly independent vectors is $\R^m$]
    Let $\bv{v}_1,\bv{v}_2,\dots,\bv{v}_m\in\R^m$ be linaerly independent. Then $\text{Span}(\bv{v}_1,\bv{v}_2,\dots,\bv{v}_m)=\R^m$.
\end{lemma}
...
\begin{defn}[D 3.13 Reduced Row Echelon Form]
    Let $R=[r_{ij}]_{i=1}^m,_{j=1}^n$ be an $m\times n$ matrix. $R$ is in RREF, if ther eis some natural number $r\leq m$ and column indices $1\leq j_1\leq j_2\leq\dots\leq n$ (*the indices of the “downward step”) such that the following two conditions hold:
    \begin{props}
...
\begin{lemma}[L 5.1.3 Linear independence of bases of orthogonal subspaces]
    Let $V$ and $W$  be two orthogonal subspaces of $\R^n$ with the bases $\bv{v}_1,\dots,\bv{v}_k$, $\bv{w}_1,\dots,\bv{w}_;$. The set of vectors $\{\bv{v}_1,\dots,\bv{v}_k,\bv{w}_1,\dots,\bv{w}_l\}$ are linearly independent.
\end{lemma}
...
\begin{defn}[D 6.4.7 CR Decomposition with pseudoinverses]
...
\begin{lemma}[L 6.4.8 Unique solution of least squares with psuedoinverses]
...
\begin{theorem}[T 6.4.10 Properties of Pseudoinverses]
    Let $A\in\R^{m\times n}$.
    \begin{compactitem}
...
\begin{lemma}[L 9.2.8 Symmetric matrix has real Eigenvalues]
    ...
    If $A\bv{v}=\lambda\bv{v}$: $$\lambda\|\bv{v}\|^2=\bar{\lambda}\bv{v}^*\bv{v}=(\lambda\bv{v})^*\bv{v}=(A\bv{v})^*\bv{v}=\bv{v}^*A^*\bv{v}=\bv{v}^*A\bv{v}=\bv{v}^*\lambda\bv{v}=\lambda\|\bv{v}\|^2$$
...
\begin{defn}[D 9.3.1 Singular Value Decomposition]
    Let $A\in\R^{m\times n}$. There exists orthogonal matrices $U\in\R^{m\times m}$ and $V\in\R^{n\times n}$ such that $U=A=U\Sigma V^\top$, where $\Sigma\in\R^{m\times n}$  is a diagonal matrix, in the sense that $\Sigma_{ij}=0$ when $i\neq j$, and the diagonal elements are non-negative and ordered in descending order. $U^\top U=I$ and $V^\top V=I$. \\
