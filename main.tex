...
\begin{defn}[D 1.7 Combination Types]
    \begin{props}
        ...
        \item \textbf{Conic:} $\lambda_i\geq 0$ for $j=1,2,\dots, n$
...
\begin{lemma}[L 1.12 Cauchy-Schwarz inequality]
    ...
    Equality if $\bv{v}\lambda=\bv{w}$ or $\bv{w}\lambda=\bv{v}$.
\end{lemma}
...
\begin{theorem}[T 2.46 CR Decomposition]
    ...
    \textbf{RREF} is simply REF without any zero rows. (i.e. in $R_0, R$ (in RREF would be $R_0$  without the last row).
\end{theorem}
...
\begin{defn}[D 4.37 Solution space]
    Set of all solutions of $A\bv{x}=\bv{b}$, thus $\text{Sol}(A,\bv{b}):=\{x\bv{x}\in\R^n:A\bv{x}=\bv{b}\}\subseteq\R^n$.
\end{defn}
...
\begin{lemma}[L 5.1.3 Linear independence of bases of orthogonal subspaces]
    Let $V$ and $W$  be two orthogonal subspaces of $\R^n$ with the bases $\bv{v}_1,\dots,\bv{v}_k$, $\bv{w}_1,\dots,\bv{w}_;$. The set of vectors $\{\bv{v}_1,\dots,\bv{v}_k,\bv{w}_1,\dots,\bv{w}_l\}$ is linearly independent.
\end{lemma}
...
\begin{theorem}[T 6.4.10 Properties of Pseudoinverse]
    ...
    Moreover, $AA^\dagger=CRR^\top(RR^\top)^{-1}(C^\top C)^{-1}C^\top = C(C^\top C)^{-1}C^\top$, which is the projection onto $\mathcal{C}(A)$, and $(AA^\dagger)^\top=AA^\dagger$.
\end{theorem}
...
\begin{note}[Operations]
    $i^2=-1$ (\textbf{NOT} $i=\sqrt{-1}$, since otherwise $1=-1$). Complex number $z_j=a_j+b_ki$. \textit{Addition, Subtraction} $(a_1\pm a_2)+(b_1\pm b_2)i$. \textit{Multiplication} $(a_1a_2-b1_b2)+(a_1b_2+a_2b_1)i$. \textit{Division} $\frac{a_1b_1+a_2b_2}{b_1^2+b_2^2}+\frac{a_2b_1-a_1b_2}{b_1^2b_2^2}i$
\end{note}
